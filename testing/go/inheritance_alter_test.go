...
GROUP BY table_name ORDER BY table_name`,
		...
		{
			Name: "SET NOT NULL preflights and propagates through descendants",
			SetUpScript: []string{
				`CREATE TABLE inherit_nn_parent (id INT, value TEXT)`,
				`CREATE TABLE inherit_nn_child (own_value TEXT) INHERITS (inherit_nn_parent)`,
				`CREATE TABLE inherit_nn_grandchild () INHERITS (inherit_nn_child)`,
				`INSERT INTO inherit_nn_parent VALUES (1, 'parent')`,
				`INSERT INTO inherit_nn_child VALUES (2, 'child', 'own')`,
				`INSERT INTO inherit_nn_grandchild VALUES (3, 'grandchild', 'grand-own')`,
			},
			Assertions: []ScriptTestAssertion{
				{Query: `ALTER TABLE inherit_nn_parent ALTER COLUMN value SET NOT NULL`},
				{Query: `SELECT table_name, is_nullable FROM information_schema.columns WHERE table_name IN ('inherit_nn_parent', 'inherit_nn_child', 'inherit_nn_grandchild') AND column_name = 'value' ORDER BY table_name`, Expected: []sql.Row{{"inherit_nn_child", "NO"}, {"inherit_nn_grandchild", "NO"}, {"inherit_nn_parent", "NO"}}},
				{Query: `ALTER TABLE inherit_nn_child ALTER COLUMN own_value SET NOT NULL`},
				{Query: `SELECT table_name, is_nullable FROM information_schema.columns WHERE table_name IN ('inherit_nn_child', 'inherit_nn_grandchild') AND column_name = 'own_value' ORDER BY table_name`, Expected: []sql.Row{{"inherit_nn_child", "NO"}, {"inherit_nn_grandchild", "NO"}}},
			},
		},
		{
			Name: "SET NOT NULL finds descendant nulls before any schema mutation",
			SetUpScript: []string{
				`CREATE TABLE inherit_nn_null_parent (id INT, value TEXT)`,
				`CREATE TABLE inherit_nn_null_child () INHERITS (inherit_nn_null_parent)`,
				`CREATE TABLE inherit_nn_null_grandchild () INHERITS (inherit_nn_null_child)`,
				`INSERT INTO inherit_nn_null_parent VALUES (1, 'present')`,
				`INSERT INTO inherit_nn_null_child VALUES (2, NULL)`,
			},
			Assertions: []ScriptTestAssertion{
				{Query: `ALTER TABLE inherit_nn_null_parent ALTER COLUMN value SET NOT NULL`, ExpectedErr: `contains null values`},
				{Query: `SELECT table_name, is_nullable FROM information_schema.columns WHERE table_name IN ('inherit_nn_null_parent', 'inherit_nn_null_child', 'inherit_nn_null_grandchild') AND column_name = 'value' ORDER BY table_name`, Expected: []sql.Row{{"inherit_nn_null_child", "YES"}, {"inherit_nn_null_grandchild", "YES"}, {"inherit_nn_null_parent", "YES"}}},
			},
		},
		{
			Name: "leaf own nullability and savepoint rollback remain physical",
			SetUpScript: []string{
				`CREATE TABLE inherit_nn_save_parent (id INT, value TEXT)`,
				`CREATE TABLE inherit_nn_save_child (own_value TEXT) INHERITS (inherit_nn_save_parent)`,
				`INSERT INTO inherit_nn_save_parent VALUES (1, 'baseline')`,
			},
			Assertions: []ScriptTestAssertion{
				{Query: `ALTER TABLE inherit_nn_save_child ALTER COLUMN own_value SET NOT NULL`},
				{Query: `ALTER TABLE inherit_nn_save_child ALTER COLUMN own_value DROP NOT NULL`},
				{Query: `BEGIN`},
				{Query: `INSERT INTO inherit_nn_save_parent VALUES (2, 'prior work')`},
				{Query: `SAVEPOINT before_set_not_null`},
				{Query: `ALTER TABLE inherit_nn_save_parent ALTER COLUMN value SET NOT NULL`},
				{Query: `ROLLBACK TO SAVEPOINT before_set_not_null`},
				{Query: `SELECT id FROM ONLY inherit_nn_save_parent ORDER BY id`, Expected: []sql.Row{{int32(1)}, {int32(2)}}},
				{Query: `SELECT table_name, is_nullable FROM information_schema.columns WHERE table_name IN ('inherit_nn_save_parent', 'inherit_nn_save_child') AND column_name = 'value' ORDER BY table_name`, Expected: []sql.Row{{"inherit_nn_save_child", "YES"}, {"inherit_nn_save_parent", "YES"}}},
				{Query: `ROLLBACK`},
				{Query: `SELECT id FROM ONLY inherit_nn_save_parent`, Expected: []sql.Row{{int32(1)}}},
			},
		},
		{
			...
			Assertions: []ScriptTestAssertion{
				...
				{Query: `ALTER TABLE inherit_required_parent ALTER COLUMN id DROP NOT NULL`, ExpectedErr: `cannot drop NOT NULL from column "id"`},
				{Query: `ALTER TABLE inherit_required_child ALTER COLUMN id DROP NOT NULL`, ExpectedErr: `cannot drop NOT NULL from inherited column "id"`},
